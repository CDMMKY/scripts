...
def k_means(path=None):
    ...
    for i in range(n_clusters):
        cluster_centers[i] = list(map(lambda x: round(x, 2), cluster_centers[i]))
    ...
    classses_in_clusters = list(indexes)
    count_cic = [defaultdict(int) for i in range(n_clusters)]
    for clst in range(n_clusters):
        for j in range(len(classses_in_clusters[clst])):
            classses_in_clusters[clst][j] = y[indexes[clst][j]]
        for cls in set(classses_in_clusters[clst]):
            count_cic[clst][cls] = classses_in_clusters[clst].count(cls)
